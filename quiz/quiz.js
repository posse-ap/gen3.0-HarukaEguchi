...
<div class="quiz-box-question">
  ...
  <figure class="quiz-box-image">
    <img src="./img/q${quizItem.id}-img.png" alt="">
  </figure>
</div>
  ...
  */
  const shuffle = arrays => {
    ...
    for (let i = array.length - 1; i >= 0; i--) { //i = ランダムに選ぶ終点のインデックス
      const randomIndex = Math.floor(Math.random() * (i + 1));  // randamIndex = 範囲内から選ぶランダム変数
      [array[i], array[randomIndex]] = [array[randomIndex], array[i]]; //分割代入で二つの変数iとrandomIndexを入れ替える
    }
    return array;
  }
  ...
  allQuiz.forEach(quiz => {
    ...
    answers.forEach(answer => {
      answer.addEventListener('click', () => {
        ...
        //  const quizArray = shuffle(ALL_QUIZ) で定義してるからここから問題をとってくる
      })
    })
  })
}